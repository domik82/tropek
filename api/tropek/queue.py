...
async def _run_change_point_phase(
    session_factory: Any,
    snapshot: Any,
    slo_def: Any,
...
async def run_evaluation_job(ctx: dict[str, Any], eval_id_str: str, defer_count: int = 0) -> None:
    ...
    await _run_change_point_phase(session_factory, snapshot, slo_def, cache, log)
...
def _sweeper_cron_seconds(interval_seconds: int) -> set[int]:
    ...
    if 60 % interval_seconds != 0:
        msg = f'finalize_sweeper_interval_seconds must divide 60; got {interval_seconds}'
