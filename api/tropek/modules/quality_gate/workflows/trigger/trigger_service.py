    ...
    async def trigger_evaluate_batch(self, request: EvaluateBatchRequest) -> EvaluateBatchResponse:
        ...
        if request.mode == 'by_date':
            ...
            pairs: list[tuple[str, datetime, datetime]] = [
                (request.asset_name, p.period_start, p.period_end) for p in request.periods
            ]
            ...
            pairs = [(name, request.period_start, request.period_end) for name in request.asset_names]
        else:
