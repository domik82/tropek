...
def _ds_params(**overrides: object) -> DataSourceCreateParams:
...
async def test_list_all(db_session: AsyncSession) -> None:
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-1'))
    await repo.create(_ds_params(name='ds-2', adapter_url='http://other:8082'))
...
async def test_delete_removes_record(db_session: AsyncSession) -> None:
    repo = DataSourceRepository(db_session)
    ds = await repo.create(_ds_params(name='to-delete'))
...
async def test_update_adapter_url(db_session: AsyncSession) -> None:
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-update'))
...
async def test_list_all_filter_by_adapter_type(db_session: AsyncSession) -> None:
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='prom-1', adapter_type='prometheus'))
    await repo.create(_ds_params(name='pg-1', adapter_type='postgres', adapter_url='http://pg:5432'))
...
async def test_list_all_filters_by_tag(db_session: AsyncSession) -> None:
    ...
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-a', adapter_url='http://a', tags={'env': 'prod'}))
    await repo.create(_ds_params(name='ds-b', adapter_url='http://b', tags={'env': 'staging'}))
...
async def test_get_tag_keys(db_session: AsyncSession) -> None:
    ...
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-a', adapter_url='http://a', tags={'env': 'prod', 'team': 'a'}))
    await repo.create(_ds_params(name='ds-b', adapter_url='http://b', tags={'env': 'staging'}))
...
async def test_get_tag_values(db_session: AsyncSession) -> None:
    ...
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-a', adapter_url='http://a', tags={'env': 'prod'}))
    await repo.create(_ds_params(name='ds-b', adapter_url='http://b', tags={'env': 'prod'}))
    await repo.create(_ds_params(name='ds-c', adapter_type='mock', adapter_url='http://c', tags={'env': 'staging'}))
...
async def test_delete_by_name_success(db_session: AsyncSession) -> None:
    ...
    repo = DataSourceRepository(db_session)
    await repo.create(_ds_params(name='ds-del', adapter_type='mock', adapter_url='http://d'))
