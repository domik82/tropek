...
def test_key_sli_fail_overrides_score(slo_fixture) -> None:
    metrics = {'response_time_p99': 550.0, 'error_rate': 1.0, 'compilation_s': 45.0}
    result = evaluate(slo_fixture('full_evaluation.yaml'), metrics, baselines={})
    assert result.result == 'fail'


def test_missing_metric_fails_objective(slo_fixture) -> None:
...
def test_relative_criteria_with_baseline_pass(slo_fixture) -> None:
    metrics = {'response_time_p99': 550.0, 'error_rate': 0.0, 'compilation_s': 45.0}
    # AND block: ["<600", "<=+10%"] — baseline=500, target=550 → both pass
    result = evaluate(
        slo_fixture('full_evaluation.yaml'),
        metrics,
...
def test_change_relative_pct_computed(slo_fixture) -> None:
    metrics = {'response_time_p99': 550.0, 'error_rate': 0.0, 'compilation_s': 45.0}
    result = evaluate(
        slo_fixture('full_evaluation.yaml'),
        metrics,
...
def test_pass_targets_included(slo_fixture) -> None:
    metrics = {'response_time_p99': 550.0, 'error_rate': 0.0, 'compilation_s': 45.0}
    result = evaluate(
        slo_fixture('full_evaluation.yaml'),
        metrics,
