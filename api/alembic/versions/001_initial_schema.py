...
Revision ID: 001
Revises:
Create Date: 2026-03-30 07:25:19.911147
...
def upgrade() -> None:  # noqa: PLR0915
    ...
    op.create_table(
        ...
        sa.Column(
            'trigger_params', postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'{}'"), nullable=False
        ),
        sa.Column(
            'evaluation_ids', postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'[]'"), nullable=False
        ),
    ...
    # slo_groups must be created before slo_definitions (FK: generated_by_group_id)
    op.create_table(
        'slo_groups',
        sa.Column('id', sa.UUID(), nullable=False),
        sa.Column('name', sa.Text(), nullable=False),
    ...
    # template_bindings must be created before slo_bindings (FK: template_binding_id)
    op.create_table(
        'template_bindings',
        sa.Column('id', sa.UUID(), nullable=False),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('pending','running','completed','failed','partial')", name='ck_evaluations_status'
        ),
