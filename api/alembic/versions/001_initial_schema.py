...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'indicators', postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'{}'"), nullable=False
        ),
    ...
    op.create_index(
        'idx_sli_definitions_latest', 'sli_definitions', ['name', sa.literal_column('version DESC')], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            ['slo_definitions.id'],
            name='fk_slo_groups_template_slo_definition_id',
            use_alter=True,
        ),
        sa.PrimaryKeyConstraint('id'),
    )
    ...
    op.create_index(
        'uq_slo_groups_name_active', 'slo_groups', ['name'], unique=True, postgresql_where=sa.text('active = true')
    )
    op.create_table(
        'asset_group_links',
    ...
    op.create_index(
        'idx_slo_definitions_latest', 'slo_definitions', ['name', sa.literal_column('version DESC')], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.CheckConstraint(
            "result IN ('pass','warning','fail','error') OR result IS NULL", name='ck_evaluations_result'
        ),
    ...
    op.create_index(
        'idx_evaluations_period', 'evaluations', ['asset_id', sa.literal_column('period_start DESC')], unique=False
    )
    ...
    op.create_index(
        ...
        unique=True,
        postgresql_where=sa.text('asset_group_id IS NOT NULL'),
    )
    op.create_table(
    ...
    op.create_table(
        ...
        sa.Column('asset_id', sa.UUID(), nullable=False),
        sa.Column(
            'asset_snapshot', postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'{}'"), nullable=False
        ),
        sa.Column('period_start', sa.DateTime(timezone=True), nullable=False),
        sa.Column('period_end', sa.DateTime(timezone=True), nullable=False),
        sa.Column('result', sa.Text(), nullable=True),
        ...
        sa.CheckConstraint(
            "result IN ('pass','warning','fail','error') OR result IS NULL", name='ck_slo_evaluations_result'
        ),
        sa.CheckConstraint(
            "status IN ('pending','running','completed','failed','partial')", name='ck_slo_evaluations_status'
        ),
        sa.ForeignKeyConstraint(['asset_id'], ['assets.id'], ondelete='RESTRICT'),
    ...
    op.create_index(
        'idx_indicator_results_objective_status', 'indicator_results', ['slo_objective_id', 'status'], unique=False
    )
    ...
    op.create_index(
        'idx_sli_values_lookup', 'sli_values', ['evaluation_name', 'metric_name', 'eval_start'], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        'uq_slo_evaluations_identity', table_name='slo_evaluations', postgresql_where=sa.text("status != 'failed'")
    )
    op.drop_index(
        'idx_slo_evaluations_stuck', table_name='slo_evaluations', postgresql_where=sa.text("status = 'running'")
    )
    ...
    op.drop_index(
        'uq_slo_group_assignments_group',
        table_name='slo_group_assignments',
        postgresql_where=sa.text('asset_group_id IS NOT NULL'),
    )
    op.drop_index(
        'uq_slo_group_assignments_asset',
    ...
    op.drop_index(
        'uq_slo_assignments_group_slo',
        table_name='slo_assignments',
        postgresql_where=sa.text('asset_group_id IS NOT NULL'),
    )
    op.drop_index(
        'uq_slo_assignments_asset_slo', table_name='slo_assignments', postgresql_where=sa.text('asset_id IS NOT NULL')
    )
