...
def main() -> None:
    """Trigger seeding evaluations and wait for all to complete."""
    if len(sys.argv) != 2:
    ...
    print(f"Triggered {len(eval_ids)} evaluations, waiting for completion...")
    ...
    print(
        f"seeded: {completed}/{len(eval_ids)} completed"
        f" — {passed} pass, {warning} warning, {failed} fail"
    )
