...
async def query_metrics(
    ...
    logger.info(
        ...
        x_datasource_name, body.start, body.end, list(body.queries.keys()),
    )

    for name, spec in body.queries.items():
        if isinstance(spec, dict) and spec.get('mode') == 'aggregated':
            _handle_aggregated(
                name, spec, body, x_datasource_name, values, errors, metadata,
            )
        else:
...
def _handle_aggregated(
    ...
    logger.info(
        ...
        name, namespace, base_value, csv_result.errors,
    )
