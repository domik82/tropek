...
- Modify: `api/alembic/versions/001_initial_schema.py` — regenerated from scratch by `db-regen-migrations.sh` (the script squashes all models into one autogenerated migration + restores the manual 002).
...
- [ ] **Step 1.2.2: Run the regeneration script.** This script tears down the test DB, squashes all ORM models into a fresh `001_initial_schema.py`, restores the manual `002`, and verifies both apply cleanly. It does NOT create an incremental `003` — the project is pre-production and rewrites migration history freely.
  Run: `./scripts/db-regen-migrations.sh`
  Expected: script completes with "Migration regeneration complete". `api/alembic/versions/001_initial_schema.py` now includes `create_table("asset_meta_snapshots")`, `create_table("asset_meta_values")`, `create_table("asset_meta_closures")`, and matching indexes alongside all existing tables.

- [ ] **Step 1.2.3: Inspect the regenerated `001_initial_schema.py`** and confirm the three new tables match §4.1 exactly (FK cascades, correct index columns, correct uniques). Do NOT edit by hand — if something is wrong, fix the ORM model and re-run the script.

- [ ] **Step 1.2.4: Apply migrations to dev DB.** The test DB was already migrated by the regen script; now do the dev DB.
  Run: `just migrate`
  Expected: "alembic upgrade head" succeeds.

- [ ] **Step 1.2.5: Commit.** The test DB was already migrated by the regen script (step 7 of the script verifies both migrations apply cleanly). No separate `just migrate-test` needed.
  ```bash
  git add api/alembic/versions/
  git commit -m "feat(db): regenerate migration with asset meta tables"
  ```
